...
def get_dma_list():
    """ 获取dma区域信息 """
    sql = 'select check_formula_nrw, check_formula, gjz, name, dmalevel from tz_dmainfo where txyx=0 and %s is not null' % FIELD_DMA_FORMULA
    return oracle.query(ctx.zlsdb, sql)


def get_dma_list_all():
    """ 获取全部dma区域信息 """
    sql = 'select check_formula_nrw, check_formula, gjz, name, dmalevel from tz_dmainfo where tx'
    return oracle.query(ctx.zlsdb, sql)
...
def zbid2sid_list(zbid_list):
    return list(filter(lambda d: d is not None, [zbid2sid(x) for x in zbid_list]))


def sid2zbid_list(sid_list):
    return list(filter(lambda d: d is not None, [sid2zbid(x) for x in sid_list]))
