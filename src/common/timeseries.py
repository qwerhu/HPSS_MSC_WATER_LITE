...
class TimeSeries:
    def __init__(self, db, bucket, interval, alignment=True):
        ...
        self.limits = self.padding / self.interval
    ...
    def upsert(self, m, _id, v, t, w=1, s=None):
        ...
        tpl = {'time': 0, 'motime': 0, 'value': 0}
        node = {'time': metric, 'motime': t, 'value': v}
        if s is not None:
            ...
            for o in self.__db().find({'id': _id, 'timetag': {'$gte': utils.pim(start, self.padding, self.interval)[0],
             ...
             (
              'timetag', sort)]):
                ...
                for oo in o['values']:
                    if start <= oo[tt] <= end:
                        return {'id': _id, 'time': oo[tt], 'value': oo['value']}
    ...
    def items(self, s=None, e=None, n=None, Ids=None, sort=[('id', 1), ('time', 1)], loose=False, ext=None):
        ...
        # # 增加demo date的处理
        # if ctx.demo_date is not None:
        #     temp_diff = e - s
        #     s = ctx.demo_date + (s % 86399) - time.timezone
        #     e = s + temp_diff
